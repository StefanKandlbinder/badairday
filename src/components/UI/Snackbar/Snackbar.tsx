...
export default function Snackbar({ header, cta }: Props) {
  ...
  return (
    <div>
      ...
      <div className="air__snackbar-wrapper" data-show={didMount}>
        <div className="air__snackbar">
          <div className="air__snackbar-header">{header}</div>
          <div className="air__snackbar-cta" onClick={onClick}>
            <Button title={cta} />
          </div>
        </div>
      </div>
